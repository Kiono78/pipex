...
/*   Updated: 2022/02/09 16:46:42 by bterral          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		execute_command(t_pipex *pipex, char **argv, char **envp, int argv_count)
{
	if (argv_count == (pipex->argc - 2) && pipex->here_doc == 0)
		pipex->outfile = open(argv[pipex->argc - 1], O_WRONLY | O_CREAT | O_TRUNC, 0644);
	...
	if (pipex->pid == 0)
	{
		...
		if (argv_count == (pipex->argc - 2))
			pipex->outfile = open_file(argv[pipex->argc - 1], O_TRUNC | O_CREAT | O_RDWR);
